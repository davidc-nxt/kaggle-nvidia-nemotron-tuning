...
train_nb = nb(accelerator="nvidiaRtxPro6000Blackwell", cells=[
    ...
    md(
        ...
        "this competition). We rely on:",
        "  * Kaggle's stock ML image: `torch`, `transformers`, `peft`, `accelerate`,",
        "    `trl`, `datasets`, `bitsandbytes`, `polars` are expected to be pre-installed.",
        "  * `ryanholbrook/nvidia-utility-script` attached as a kernel source — provides",
        "    the CUTLASS DSL bundle and (in some images) `mamba_ssm` / `causal_conv1d`.",
        "",
        "**Required GPU**: **GPU RTX Pro 6000** (48 GiB single GPU) or **GPU T4 x2** if",
        "Internet is on. P100 (sm_60) is not supported by current PyTorch.",
    ),
    code(
        "# Probe what's actually available BEFORE doing anything destructive.",
        "import importlib, sys",
        "# bitsandbytes intentionally omitted — we load in BF16 on the 95 GiB RTX Pro 6000.",
        "_required = ['torch', 'transformers', 'peft', 'accelerate', 'trl', 'datasets',",
        "             'polars', 'mamba_ssm', 'causal_conv1d']",
        "_missing = []",
        "for mod in _required:",
        "    try:",
        "        importlib.import_module(mod)",
        "    except Exception as e:",
        "        _missing.append((mod, type(e).__name__, str(e)[:120]))",
        "if _missing:",
        "    print('MISSING PACKAGES (will try to recover via attached kernel sources):')",
        "    for m, et, msg in _missing:",
        "        print(f'  {m:18s}  {et}: {msg}')",
        "else:",
        "    print('all required packages present in the base image')",
    ),
    md(
    ...
    code(
        ...
        "",
        "# `ryanholbrook/nvidia-utility-script` (attached as kernel_source) installs",
        "# nightly torch + cutlass + causal_conv1d + mamba_ssm directly into /kaggle/working,",
        "# which becomes /kaggle/usr/lib/notebooks/<owner>/<slug>/ when attached. Add it",
        "# to sys.path with high precedence so its packages win against the base image.",
        "import sys",
        "_utility_root = '/kaggle/usr/lib/notebooks/ryanholbrook/nvidia-utility-script'",
        "if os.path.exists(_utility_root):",
        "    if _utility_root not in sys.path:",
        "        sys.path.insert(0, _utility_root)",
        "    site.addsitedir(_utility_root)  # also process any .pth files",
        "    print(f'attached utility-script packages at {_utility_root}')",
        "    print(f'  contains mamba_ssm: {os.path.isdir(_utility_root + \"/mamba_ssm\")}')",
        "    print(f'  contains causal_conv1d: {os.path.isdir(_utility_root + \"/causal_conv1d\")}')",
        "else:",
        "    print(f'WARNING: {_utility_root} does not exist; mamba_ssm imports will fail')",
        "",
        "# Re-verify the imports that previously failed.",
        "import importlib",
        "for mod in ('mamba_ssm', 'causal_conv1d'):",
        "    try:",
        "        m = importlib.import_module(mod)",
        "        print(f'  OK: {mod} from {getattr(m, \"__file__\", \"?\")}')",
        "    except Exception as e:",
